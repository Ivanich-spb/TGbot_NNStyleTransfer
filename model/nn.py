...
def gram_matrix(input_data):
    batch_size, h, w, f_map_num = input_data.size()  # batch size(=1)
    # b=number of feature maps
    # (h,w)=dimensions of a feature map (N=h*w)

    features = input_data.view(batch_size * h, w * f_map_num)  # resize F_XL into \hat F_XL
...
def start(content_img_path: str, style_img_path: str) -> str:
    style_img = image_loader(style_img_path)  # as well as here
    content_img = image_loader(content_img_path)  # измените путь на тот который у вас.
    input_img = content_img.clone()
    output = run_style_transfer(content_img, style_img, input_img)
    postpb = transforms.Compose([transforms.ToPILImage()])
    path = content_img_path[:-7] + "result.png"
    postpb(output.squeeze()).save(path)
    #print(path)
    clear_ram()
    return path
